    ...
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            ...
            if (req.cookies["X-Application-Access"]) {
                const access = ALTdcp<UserE | null>(req.cookies["X-Application-Access"], config.server.accessTokenSecret, req.access.ip?.toString());
                if (access) {
                    ...
                    if (access?.suspended) {
                        ...
                        switch (req.accepts(["html", "json", "txt"])) {
                            case "html":
                                return res.status(401).send(SenderError({ lang: access.lang || "unk", status: 401, title: i18n.t("react:titles.Suspended"), code: i18n.t("react:messages.SuspendedAccount"), message: i18n.t("attributes.motive") + access.suspendedReason || i18n.t("react:messages.DefaultSuspendedAccoutReason") }, req))
                            case "json":
                                return res.status(401).json({
                                    type: "error",
                                    message: access.suspendedReason || i18n.t("react:messages.DefaultSuspendedAccoutReason"),
                                    status: res.statusCode || req.statusCode || "unknown",
                                    timestamp: Date.now(),
                                });
                            default:
                                return res.status(401).type("txt").send(i18n.t("react:messages.DefaultSuspendedAccoutReason"));
                        }
                    }
                }
            } else if (!req.cookies["X-Application-Access"] && req.cookies["X-Application-Refresh"] && typeof req.cookies["X-Application-Refresh"] === "string") {
                const refresh = ALTdcp<{ remember: string } | null>(req.cookies["X-Application-Refresh"], config.server.refreshTokenSecret, req.access.ip?.toString());
                if (refresh) {
                    ...
                    if (userRecord) {
                        ...
                        delete UserData.id;
                        // Atualiza o Access
                        const access = ALTcpt(UserData, config.server.accessTokenSecret, { ip: req.access.ip?.toString(), expires: "15m" })
                        ...
                        // atualiza o Refresh
                        const token = ALTcpt({ remember: rememberMeUUID }, config.server.refreshTokenSecret, { ip: req.access.ip?.toString(), expires: "90d" })
