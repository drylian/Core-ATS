...
export function Connections(core: LoggingsMethods) {
    ...
    const BlockedList = [
        ".php", ".env", ".git/config", ".PHP", "ENV=",
        ".asp", ".aspx", ".jsp", ".xml",
        ".htaccess", ".bak", ".swp", ".swo",
        ".sql", ".csv", ".sqlite", ".db",
        "/admin", "/wp-admin", "/wp-login.php", "/wp-content",
        "/wp-includes", "/phpmyadmin", "/myadmin", "/manager",
        "?cmd=", "?exec=", "?shell=", "?download=",
        "?upload=", "?php=", "?eval=", "?sql=",
        "config.php", "database.php", "settings.php",
        "web.config", "phpinfo.php", "info.php",
        "phpMyAdmin.config.php", "wp-config.php",
        "configuration.php", "php.ini", "passwd",
        "shadow", "ssh_config", "known_hosts",
        "authorized_keys", "id_rsa", "id_dsa",
        "secret_key", "private.key",
        "wp-config.bak", "wp-config.old",
        "wp-config.zip", "wp-config.bak.php",
        "wp-config.old.php", "wp-config.zip.php"
    ];
    ...
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        function CheckerIps(message: string, blockedRoute?: boolean) {
            if (req.access.ip !== undefined) {
                if (ip[req.access.ip?.toString()]) storage.set("Accesslist", { [req.access.ip?.toString()]: { requests: ip[req.access.ip?.toString()].requests += 1 } }, true)
                if (blockedRoute) {
                    if (ip[req.access.ip?.toString()]) {
                        const data = ip[req.access.ip?.toString()].warn += 1;
                        const timestamp = Date.now();

                        storage.set("Accesslist", { [req.access.ip?.toString()]: { warn: data, history: { [timestamp.toString()]: req.originalUrl } } }, true)
                    } else {
                    ...
                    if (ip[req.access.ip?.toString()].warn <= 3) {
                        ...
                        if (!ip[req.access.ip?.toString()].blocked) core.warn(`[Banido].gray: [${req.originalUrl}].red [IP:].yellow [(${req.access.ip?.toString()})].red`);
                ...
                if (ip[req.access.ip?.toString()] && ip[req.access.ip?.toString()].blocked) {
                    banned.warn(`Tentativa de acesso [${req.originalUrl}].red com o ip banido [(${req.access.ip?.toString()})].red`)
                    return res.destroy();
                }
                else {
                    core.debug(message);
        ...
        if (req.originalUrl === "/robots.txt") {
            ...
            return CheckerIps(`[Bloqueado].gray: [${req.originalUrl}].red [IP:].yellow [(${req.access.ip?.toString()})].red - [Motivo].gray : A Request acessada era uma request proibida.`, true);
        }
        ...
        CheckerIps(`Acessos: [${req.originalUrl}].magenta [(${extend === 0 ? "" + callCount++ : extend + " mil e " + callCount++})].blue`);
    };
}
