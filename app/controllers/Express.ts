...
class Express {
	...
	private middlewares(): void {
		...
		this.server.use(TokensValiter());
		this.server.use(cookieParser(this.config.server.csrf.cookie_secret));
		this.server.use(Credentials());
		/**
         * Core Middlewares
         */
		this.server.use(SenderSettings()); // adiciona o res.sender
		this.server.use(Cors()); // sender necessario
		this.server.use(Protocols()); // sender necessario
		this.server.use(Connections(this.core)); // sender e Protocols necessario
	}
