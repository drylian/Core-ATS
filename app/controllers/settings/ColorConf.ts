...
export default async function ColorConf(core: LoggingsMethods) {
    ...
    try {
        ...
        if (!black?.color?.secondary)
            jsonsv(root.configPATH + '/color.json', { black: { color: { secondary: '#1d5285' } } });
        if (!black?.color?.tertiary)
            jsonsv(root.configPATH + '/color.json', { black: { color: { tertiary: '#808080' } } });
        if (!black?.text?.primary) jsonsv(root.configPATH + '/color.json', { black: { text: { primary: '#ffffff' } } });
        if (!black?.text?.secondary)
            jsonsv(root.configPATH + '/color.json', { black: { text: { secondary: '#051322' } } });
        ...
        if (!white?.color?.secondary)
            jsonsv(root.configPATH + '/color.json', { white: { color: { secondary: '#1d5285' } } });
        if (!white?.color?.tertiary)
            jsonsv(root.configPATH + '/color.json', { white: { color: { tertiary: '#808080' } } });
        if (!white?.text?.primary) jsonsv(root.configPATH + '/color.json', { white: { text: { primary: '#fff' } } });
        if (!white?.text?.secondary)
            jsonsv(root.configPATH + '/color.json', { white: { text: { secondary: '#1d5285' } } });
