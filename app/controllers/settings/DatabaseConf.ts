import { jsonsv, json, dirEX, dirCR, question } from "@/utils";
...
export default async function DatabaseConf(core: LoggingsMethods) {
    core.sys("Iniciando verificações do [Banco de dados].blue do painel [3/4]");
    ...
    if (!database?.database?.dialect) jsonsv(root.configPATH + "/settings.json", { database: { dialect: "sqlite" } });
