export function getTimestamp() {
	...
	const currentHour = `${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}:${String(now.getSeconds()).padStart(2, "0")}`;

	// Obter a data atual no formato [ DDD/MMM/AAAA, ás HH:MM:SS ]
	const fulltimer = `${String(now.getDate()).padStart(2, "0")}/${String(now.getMonth() + 1).padStart(2, "0")}/${now.getFullYear()}, ás ${currentHour}`;
